...
public class MessageDemoProvider {
    ...
    /**
     *  包含发送消息以及添加 confirm 监听、添加 return 监听。如果消费端要设置为手工 ACK ，那么生产端发送消息的时候一定发送 correlationData ，并且全局唯一，用以唯一标识消息
     */
    @Resource
    ...
    private final RabbitTemplate.ReturnsCallback returnsCallback = new RabbitTemplate.ReturnsCallback(){
        ...
        public void returnedMessage(ReturnedMessage returnedMessage) {
            log.info("【常规消息】消息经交换器发送到队列失败触发，回调参数：{}",returnedMessage.toString());
        }
    };
